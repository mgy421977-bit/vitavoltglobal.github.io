/* Vitavolt Global — VITA Engine Offline Pre-Feasibility Core | build 2026-09-17-v3-inv-fit */
(function (window) {
  'use strict';
  var ENGINE_VERSION = '3.0.1', BUILD = '2026-09-17-v3-inv-fit', DB_VERSION = '1.9';
  var CONFIG = {
  ...
  function autoInverter(dcKwp, pricing) {
    ...
    for (var i = 0; i < a.length; i++) {
      var p = +a[i].power_kw;
      if (p >= dcKwp && p <= singleCap) {
        return { opt: a[i], reason: 'smallest_catalog_unit_covering_dc_kwp', count: 1 };
      }
    }
    var best = null;
    for (var j = 0; j < a.length; j++) {
      ...
      if (!best || excess < best.excess - 0.05 ||
          (Math.abs(excess - best.excess) <= 0.05 && count < best.count) ||
          (Math.abs(excess - best.excess) <= 0.05 && count === best.count && cost < best.cost)) {
        best = cand;
      }
    }
    if (!best) {
    ...
    return {
      opt: best.opt,
      reason: best.count === 1 ? 'catalog_unit_covering_dc_kwp' : 'parallel_units_min_excess_covering_dc_kwp',
      count: best.count
    };
  }
  ...
  function buildBom(dc, count, pp, invSel, bessReq, pricing) {
    ...
    function row(category, item, quantity, unit, unitCost, source, costStatus) {
      var q = nn(quantity);
      var uc = unitCost == null ? null : +unitCost;
  ...
  function pricing(input, pricingCfg) {
    var c = pricingCfg || CONFIG.pricing;
    var cm = c.cost_model || CONFIG.pricing.cost_model;
    var pm = cm.proportional_model || {};
    ...
    var po = panelOpt(pp, c);
    var pr = Math.max(0, n(po.base_usd_per_w));
    var panelCost = round2(cnt * pp * pr);
    var invSel = autoInverter(dc, c);
    var inv = invSel.opt;
    var invCost = inv ? round2(invSel.count * n(inv.base_usd)) : 0;
    var bessReq = Math.max(0, n(input.bessCapacityKwh));
    var bu = batteryUnit(c);
    var bUnits = bessReq > 0 ? Math.ceil(bessReq / +bu.capacity_kwh) : 0;
    var installed = round2(bUnits * +bu.capacity_kwh);
    var battCost = round2(bUnits * n(bu.base_usd, 800));
    var laborRate = Math.max(0, n(cm.labor_usd_per_panel, 20));
    var labor = round2(cnt * laborRate);
    var bos = round2(dc * n(pm.bos_usd_per_kwp, 0) + n(pm.fixed_bos_usd, 0));
    var directMaterial = round2(panelCost + invCost + battCost);
    var directCost = round2(directMaterial + labor);
    var projectCost = round2(directCost + bos);
    var markup = Math.max(0, n(c.markup_pct, 15));
    var sales = round2(directCost * (1 + markup / 100));
    var bom = buildBom(dc, cnt, pp, invSel, bessReq, c);
    return {
      ...
      inverter: { selection: 'auto', powerKw: inv ? +inv.power_kw : 0, type: inv ? inv.type : null, count: invSel.count, unitCost: inv ? +inv.base_usd : 0, baseUsd: invCost, source: inv ? inv.source : 'DATABASE', selectionReason: invSel.reason },
      battery: { requestedCapacityKwh: round2(bessReq), installedCapacityKwh: installed, batteryModuleCount: bUnits, batteryCost: battCost, requestedKwh: round2(bessReq), installedKwh: installed, units: bUnits, baseUsd: battCost, referenceUnitKwh: +bu.capacity_kwh, referenceUnitUsd: +bu.base_usd, basis: 'discrete_2.4kWh_modules_ceiling', provenance: provenance(+bu.base_usd, 'USD/module', 'DATABASE') },
      ...
      commercialPrice: { usd: sales, basis: 'direct_cost_plus_commercial_margin' },
      salesPrice: { usd: sales, markupPct: markup },
      bom: bom,
      marketPanelUsd: panelCost,
      marketInverterUsd: invCost,
      marketPanelInverterUsd: round2(panelCost + invCost)
    };
  }
  function calculateWater(input, selected) {
    ...
    var roof = nn(input.roofAreaM2), m = nn(input.monthlyWaterM3), r = nn(input.rainfallMm || rainfall(input.city));
    var t = input.roofType || CONFIG.water.defaultRoofType;
    var c = CONFIG.water.roofRunoffCoefficient[t] || 0.75;
    var q = CONFIG.water.firstFlushAndOverflowFactor;
    var demand = m * 12;
    var rain = { selected: false, status: 'NOT_SELECTED' };
    if (wantRain && roof > 0) {
      var theo = (roof * r) / 1000 * c, usable = theo * q;
      rain = { selected: true, status: 'CALCULATED', city: input.city || null, rainfallMm: +r.toFixed(1), roofAreaM2: +roof.toFixed(1), roofType: t, annualTheoreticalM3: +theo.toFixed(1), annualUsableM3: +usable.toFixed(1), demandCoveragePct: demand ? +Math.min(100, (usable / demand) * 100).toFixed(1) : 0 };
    }
    var grey = { selected: false, status: 'NOT_SELECTED' };
    if (wantGrey && m > 0) {
      var gm = m * CONFIG.water.greywaterRecoverableRatio, gy = gm * 12 * CONFIG.water.greywaterOperatingLossFactor;
      grey = { selected: true, status: 'CALCULATED', monthlySourceM3: +gm.toFixed(2), annualUsableM3: +gy.toFixed(1), demandCoveragePct: demand ? +Math.min(100, (gy / demand) * 100).toFixed(1) : 0 };
    }
  ...
  function validate(result) {
    var issues = [], s = result.solar || {}, p = result.pricing || {};
    var expectedDc = round2(((s.panelCount || 0) * ((result.assumptions && result.assumptions.panelPowerWp) || 620)) / 1000);
    ...
    if (p.battery && p.battery.units != null) {
      var inst = round2(p.battery.units * 2.4);
      ...
      var cost = round2(p.battery.units * 800);
      if (Math.abs((p.battery.baseUsd || 0) - cost) > 0.05) issues.push({ check: 'BESS_COST', expected: cost, actual: p.battery.baseUsd });
    }
    if (p.panel && s.panelCount) {
      var pc = round2(s.panelCount * ((result.assumptions && result.assumptions.panelPowerWp) || 620) * (p.panel.baseUsdPerW || 0.18));
      if (Math.abs((p.panel.baseUsd || 0) - pc) > 0.05) issues.push({ check: 'PANEL_COST', expected: pc, actual: p.panel.baseUsd });
    }
    if (p.labor && s.panelCount) {
      var lab = round2(s.panelCount * 20);
      if (Math.abs((p.labor.baseUsd || 0) - lab) > 0.05) issues.push({ check: 'LABOR', expected: lab, actual: p.labor.baseUsd });
    }
    var factor = (result.assumptions && result.assumptions.co2FactorKgPerKwh) || 0.42;
    var co2 = Math.round((s.annualProductionKwh || 0) * factor);
  ...
  function calculate(input, cfg) {
    input = input || {};
    cfg = cfg || {};
    var solarCfg = Object.assign({}, CONFIG.solar, cfg.solar || {});
    var battCfg = Object.assign({}, CONFIG.battery, cfg.battery || {});
    var pricingCfg = Object.assign({}, CONFIG.pricing, cfg.pricing || {});
    ...
    var roof = nn(input.roofAreaM2), land = nn(input.landAreaM2);
    var area = roof + (input.landAvailable ? land : 0);
    var panelArea = Math.max(0.5, n(solarCfg.panel_area_m2, 2.6));
    var pw = Math.max(1, n(input.panelPowerWp != null ? input.panelPowerWp : solarCfg.default_panel_power, 620));
    var count = Math.max(0, Math.floor(area / panelArea));
    var dc = round2((count * pw) / 1000);
    var yieldKwh = Math.max(300, n(solarCfg.default_specific_yield_kwh_kwp, 1450));
    var loss = clamp(n(solarCfg.system_loss_factor, 0.85), 0.1, 1);
    var pr = clamp(n(solarCfg.performance_ratio, 0.8), 0.1, 1);
    var factor = Math.max(0, n(solarCfg.co2_factor, 0.42));
    var margin = clamp(n(solarCfg.design_margin, 1.1), 0.5, 2);
    var annual = nn(input.annualConsumptionKwh);
    if (!annual && input.monthlyConsumptionKwh) annual = nn(input.monthlyConsumptionKwh) * 12;
    var production = Math.round(dc * yieldKwh * loss * pr);
    var ratio = annual ? Math.min(1, Math.max(0, (annual * margin) / Math.max(production, 1))) : 0;
    var self = Math.min(annual, production, Math.round(production * ratio));
    var exp = Math.max(0, production - self);
    var co2 = Math.round(production * factor);
    var daily = annual / 365;
    var night = input.nighttimeShare != null ? clamp(n(input.nighttimeShare), 0, 1) : 0.35;
    var peak = nn(input.peakDemandKw);
    var rec = night >= 0.4 || peak >= Math.max(20, dc * 0.35);
    var bess = 0;
    if (rec && daily > 0) {
      var target = daily * night * 0.75;
      var dod = clamp(n(battCfg.depth_of_discharge, 0.9), 0.5, 1);
      var rte = clamp(n(battCfg.round_trip_efficiency, 0.95), 0.5, 1);
      bess = +(target / Math.max(dod * rte, 0.25)).toFixed(1);
    }
    if (input.bessCapacityKwh != null && nn(input.bessCapacityKwh) > 0) {
      bess = nn(input.bessCapacityKwh);
      rec = true;
    }
    var price = pricing(Object.assign({}, input, { panelPowerWp: pw, panelCount: count, dcCapacityKwp: dc, bessCapacityKwh: bess }), pricingCfg);
    var water = calculateWater({ city: input.city, roofAreaM2: roof, monthlyWaterM3: nn(input.monthlyWaterM3), roofType: input.roofType }, { rainwater: input.rainwaterSelected === true || (nn(input.monthlyWaterM3) >= 0 && roof > 0), greywater: input.greywaterSelected === true || nn(input.monthlyWaterM3) > 0 });
    if (price.bom && water) {
      if (water.rainfall && water.rainfall.selected) {
        price.bom.push({ category: 'WATER', item: 'Yağmur suyu hasat sistemi (ön değerlendirme)', quantity: 1, unit: 'sistem', unitCost: null, totalCost: null, source: 'ASSUMPTION', costStatus: 'NOT_PRICED' });
        price.bom.push({ category: 'WATER', item: 'Yağmur suyu yıllık kullanılabilir', quantity: water.rainfall.annualUsableM3 || 0, unit: 'm3', unitCost: null, totalCost: null, source: 'DERIVED', costStatus: 'NOT_PRICED' });
      }
      if (water.greywater && water.greywater.selected) {
        price.bom.push({ category: 'WATER', item: 'Gri su dönüşüm sistemi (ön değerlendirme)', quantity: 1, unit: 'sistem', unitCost: null, totalCost: null, source: 'ASSUMPTION', costStatus: 'NOT_PRICED' });
        price.bom.push({ category: 'WATER', item: 'Gri su yıllık kullanılabilir', quantity: water.greywater.annualUsableM3 || 0, unit: 'm3', unitCost: null, totalCost: null, source: 'DERIVED', costStatus: 'NOT_PRICED' });
      }
    }
    var result = {
      ...
      inputs: { roofAreaM2: roof, landAreaM2: land, landAvailable: input.landAvailable === true, annualConsumptionKwh: annual, city: input.city || null },
      ...
      assumptions: { panelPowerWp: pw, panelAreaM2: panelArea, specificYieldKwhKwp: yieldKwh, systemLossFactor: loss, performanceRatio: pr, co2FactorKgPerKwh: factor, designMargin: margin },
      ...
      contracts: { anne: { role: 'scenario_and_interpretation_only', mustNot: 'recompute_engineering' }, mitos: { role: 'multi_scenario_optimization_proposal', mustNot: 'override_engine_math' } }
    };
  ...
  function loadMarketData() {
    ...
    return fetch('/database/calculations.json?v=' + encodeURIComponent(BUILD), { credentials: 'same-origin', cache: 'no-store' })
      .then(function (r) { if (!r.ok) throw new Error('fetch failed'); return r.json(); })
      .then(function (j) {
        if (j.pricing) {
          CONFIG.pricing = Object.assign({}, CONFIG.pricing, j.pricing);
          if (j.pricing.cost_model) CONFIG.pricing.cost_model = Object.assign({}, CONFIG.pricing.cost_model, j.pricing.cost_model);
        }
