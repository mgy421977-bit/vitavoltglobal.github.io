...
(function (window, document) {
  ...
  /* Quick feasibility Excel attachment module. It installs itself after this object is created. */
  try {
    var excelLoader = document.createElement('script');
    excelLoader.src = 'js/excel-mail.js?v=2026-09-17-excel';
    excelLoader.async = true;
  ...
  function formToObject(form) {
    var data = new FormData(form), obj = {};
    data.forEach(function (value, key) {
      if (key === 'kvkk_consent') { obj[key] = true; return; }
  ...
  function customerEmail(obj) { return (obj && (obj.eposta || obj.email || obj.Email || '')) || ''; }
  function customerName(obj) { return (obj && (obj.ad_soyad || obj.name || obj.Name || '')) || ''; }

  function buildMailto(form, options) {
    options = options || {};
    var obj = formToObject(form), lines = [];
    Object.keys(obj).forEach(function (k) { if (k.charAt(0) !== '_') lines.push(k + ': ' + obj[k]); });
    var subject = options.subject || ('Vitavolt web talebi — ' + (customerName(obj) || 'Yeni lead'));
    return 'mailto:' + encodeURIComponent(FORM_CONFIG.mailtoAddress) + '?subject=' + encodeURIComponent(subject) + '&body=' + encodeURIComponent(lines.join('\n'));
  }

  function attachDeliveryFields(payload, options) {
    options = options || {};
    var name = customerName(payload), email = customerEmail(payload);
  ...
  function submitForm(form, options) {
    options = options || {};
    if (!form || !validate(form)) return Promise.resolve({ ok: false, reason: 'validation' });
    ...
    status(statusEl, '', '');
    var payload = attachDeliveryFields(formToObject(form), options);
    ...
    if (!endpoint) return new Promise(function (resolve) { setTimeout(function () { resolve(finishMailto()); }, 200); });
    return fetch(endpoint, { method: 'POST', headers: {'Content-Type':'application/json','Accept':'application/json'}, body: JSON.stringify(payload) })
      ...
      .catch(function () {
        if (FORM_CONFIG.mailtoFallback) return finishMailto();
        setLoading(btn, false);
  ...
  function submitPayload(payload, options) {
    ...
    return fetch(FORM_CONFIG.endpoint, { method:'POST', headers:{'Content-Type':'application/json','Accept':'application/json'}, body:JSON.stringify(data) })
      .then(function (res) {
  ...
  window.VitavoltForms = { validate:validate, setLoading:setLoading, status:status, track:track, formToObject:formToObject, buildMailto:buildMailto, buildAutoresponse:buildAutoresponse, buildInvestorAutoresponse:buildInvestorAutoresponse, submitForm:submitForm, submitPayload:submitPayload, config:FORM_CONFIG };
})(window, document);
