/* Vitavolt Global — ANNE CORE | build 2026-09-17-anne-v5
...
(function (window) {
  'use strict';
  var VERSION = '0.5.0-offline';
  var BUILD = '2026-09-17-anne-v5';
  ...
  function analyzeSystem(raw) {
    ...
    return { domain: 'energy_prefeasibility', facilityType: raw.facilityType || raw.tesisTipi || null, city: raw.city || null, needs: needs, requiredEngine: 'VITA_ENGINE', mitosTasks: ['completeInputs', 'propose_scenarios'], principle: 'First ensure deterministic engine; then MITOS fills inputs; ANNE tests; human approves.' };
  }
  function buildAssessment(input, result, mitosMeta) {
    ...
    if (present(input.monthlyWaterM3)) { supplied.push('Aylık su tüketimi'); factors.push({ id: 'water', label: 'Su verimliliği ve yeniden kullanım', reason: 'Su tüketim verisi mevcut.' }); }
    if (present(input.facilityType) || present(input.tesisTipi)) evidence.push({ type: 'facility', value: input.facilityType || input.tesisTipi, source: 'USER_SUPPLIED' });
    return { version: VERSION, build: BUILD, status: 'ANNE_VITA_VALIDATED', facility: { type: input.facilityType || input.tesisTipi || null, city: input.city || null }, suppliedData: unique(supplied), missingData: missing, optimizationFactors: factors, evidence: evidence, mitos: mitosMeta || { status: 'NOT_LOADED' }, confidence: missing.length === 0 ? 'DATA_COMPLETE_FOR_CURRENT_SCOPE' : 'PRELIMINARY_MISSING_DATA', principle: 'MITOS completes inputs; VITA calculates; ANNE tests deterministically; Human approves.' };
  }
  function test(result, input) {
    var checks = [];
    function add(id, ok, detail) { checks.push({ id: id, ok: !!ok, detail: detail || '' }); }
    if (!result || !result.solar || !result.pricing) return { ok: false, status: 'INCOMPLETE_RESULT', checks: [{ id: 'structure', ok: false, detail: 'missing solar/pricing' }] };
    ...
    if (p.battery && n(p.battery.requestedCapacityKwh) > 0) { var units = Math.ceil(n(p.battery.requestedCapacityKwh) / 2.4); add('BESS_MODULES', n(p.battery.batteryModuleCount) === units, 'modules=' + p.battery.batteryModuleCount + ' expected=' + units); } else add('BESS_MODULES', true, 'no BESS request');
    if (p.inverter && n(s.dcCapacityKwp) > 0) {
      var dc = n(s.dcCapacityKwp), ac = n(p.inverter.powerKw) * n(p.inverter.count, 1), target = n(p.inverter.targetAcDcRatio, 0.80);
      var withinDc = ac <= dc + 0.05;
      var preferred = ac >= dc * target - 0.05;
      var fallback = p.inverter.sizingFallback === true;
      add('INVERTER_DC_AC_RULE', withinDc ? (preferred || fallback) : fallback, 'dc=' + dc + ' ac=' + ac + ' target=' + Math.round(dc * target * 100) / 100 + ' fallback=' + fallback);
    }
    if (input && n(input.roofAreaM2, 0) > 0) add('ROOF_LOSS_TAG', input.roofOrientationLossPct == null || n(input.roofOrientationLossPct) === 20, 'roofLossPct=' + input.roofOrientationLossPct);
    if (result.water && result.water.rainfall && result.water.rainfall.selected) { var hasWaterBom = (p.bom || []).some(function (x) { return x.category === 'WATER'; }); add('WATER_BOM', hasWaterBom, hasWaterBom ? 'WATER lines present' : 'missing WATER BOM'); }
  ...
  function runPipeline(rawInput, options) {
    ...
    var pack = (window.MitosCore && typeof window.MitosCore.completeInputs === 'function') ? window.MitosCore.completeInputs(rawInput) : { status: 'MITOS_MISSING', readyForVita: true, input: rawInput || {}, sources: {}, missing: [] };
    var input = pack.input || rawInput || {};
    var mitosProposal = (window.MitosCore && typeof window.MitosCore.propose === 'function') ? window.MitosCore.propose(input) : null;
    ...
    var evaluated = [];
    if (mitosProposal && mitosProposal.scenarios) mitosProposal.scenarios.slice(1).forEach(function (s) { try { var c = window.VitaEngine.calculate(s.input, options.vitaConfig || {}); evaluated.push({ id: s.id, label: s.label, reason: s.reason, validation: c.validation || null, solar: c.solar || null, bess: c.bess || null, pricing: c.pricing || null, water: c.water || null }); } catch (e) { evaluated.push({ id: s.id, label: s.label, reason: s.reason, status: 'REJECTED', error: e.message }); } });
    ...
    result.anne = buildAssessment(input, result, mitosMeta); result.anne.systemAnalysis = analysis; result.anne.test = anneTest; result.anne.humanGate = { required: true, status: 'PENDING_HUMAN_APPROVAL', rule: 'No project decision without human approval' };
  ...
  function proposeScenario(input, objective) { return { status: 'PROPOSAL_ONLY', objective: objective || 'optimize_total_project_outcome', source: 'ANNE_CORE', input: input || {}, nextStep: 'MITOS completeInputs → VITA calculate → ANNE test → Human', humanApprovalRequired: true }; }
  function outputPlan(result, audience) { return { audience: audience || 'management', dataSource: 'validated_vita_result', sections: ['executive_summary', 'technical_design', 'bom', 'carbon', 'water', 'anne_test', 'mitos_sources'], presentation: { formats: ['PPTX', 'PDF'], status: 'CAPABILITY_READY' }, rule: 'Do not fabricate numerical values; use validated VITA outputs only.' }; }
