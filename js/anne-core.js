/* Vitavolt Global — ANNE CORE Offline Orchestrator | build 2026-09-17-v2 */
(function (window) {
  'use strict';

  var VERSION = '0.2.0-offline';
  var BUILD = '2026-09-17-anne-v2';
  ...
  function buildAssessment(input, result) {
    ...
    if (present(input.fuelConsumption)) factors.push({ id: 'fuel', label: 'Yakıt / proses enerjisi optimizasyonu', reason: 'Yakıt verisi mevcut.' });
    if (present(input.machineData)) factors.push({ id: 'equipment', label: 'Makine ve proses ekipmanı optimizasyonu', reason: 'Ekipman verisi mevcut.' });
    if (present(input.productionData)) factors.push({ id: 'process_efficiency', label: 'Üretim prosesi enerji yoğunluğu', reason: 'Üretim verisi mevcut.' });
    if (present(input.emissionData)) factors.push({ id: 'carbon', label: 'Karbon / emisyon optimizasyonu', reason: 'Emisyon verisi mevcut.' });
    if (present(input.etsData)) factors.push({ id: 'ets', label: 'ETS maruziyeti ve karbon yükümlülüğü', reason: 'ETS verisi sağlandı; güncel mevzuat ayrıca doğrulanmalı.' });
    ...
    if (present(input.processDescription)) {
      ...
      factors.push({ id: 'process_specific', label: 'Tesis prosesine özel optimizasyon', reason: 'Proses açıklaması mevcut; alt faktörler sonraki aşamada çıkarılabilir.' });
    }
  ...
  function health() {
    var vita = !!(window.VitaEngine && typeof window.VitaEngine.calculate === 'function');
    return {
      anne: true,
      vitaEngineAvailable: vita,
      status: vita ? 'READY' : 'WAITING_FOR_VITA_ENGINE',
      version: VERSION,
      build: BUILD
    };
  }
  ...
  function assess(input, options) {
    options = options || {};
    var h = health();
    if (!h.vitaEngineAvailable) throw new Error('VITA Engine is required by ANNE Core');

    var result = window.VitaEngine.calculate(input || {}, options.vitaConfig || {});
    if (!result || !result.solar || !result.pricing || !result.validation) {
      throw new Error('ANNE Core: VITA Engine returned an incomplete result');
    }

    var assessment = buildAssessment(input || {}, result);
    result.anne = assessment;
    result.anne.health = h;
    ...
    result.flow = ['USER_INPUT', 'ANNE_CORE', 'VITA_ENGINE', 'VALIDATOR', 'ANNE_REVIEW', 'HUMAN_APPROVAL'];
    return result;
  }
